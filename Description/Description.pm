...
$VERSION = q[$Id: Description.pm,v 1.8 2002/10/13 21:56:05 jason_e_stewart Exp $];

sub initialize {
  my $self = shift;
  $self->database_list([]);

  $self->mageml_lists([Database=>$self->database_list]);
...
Bio::MAGE::Description - Container module for classes in the MAGE Description package 
...
This method handles the list for the C<Bio::MAGE::ARRAY(0x104d9b00)::Database> class. It returns a reference to an array of the
...
=item $val = $description->database_list($val)

This is the unified setter/getter method for the database_list slot. If
...
=item $val = $description->mageml_lists()

=item $val = $description->mageml_lists($val)
...
=item $val = $description->tagname($val)
