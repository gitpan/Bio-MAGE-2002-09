...
#
##############################
package Bio::MAGE::AuditAndSecurity::SecurityGroup;
...
$VERSION = q[$Id: SecurityGroup.pm,v 1.4 2002/08/24 16:12:13 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::AuditAndSecurity::SecurityGroup
...
=head2 SYNOPSIS
  ...
  my $securitygroup = Bio::MAGE::AuditAndSecurity::SecurityGroup->new(members=>$members_value);

    # setting and retrieving object associations
  my $members_val = $securitygroup->members();
  $securitygroup->members($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<SecurityGroup> class:
...
Bio::MAGE::AuditAndSecurity::SecurityGroup has the following super classes
...
instance of the class via the Bio::MAGE::AuditAndSecurity::SecurityGroup->methodname() syntax.
...
Sets the value of the members association (from C<Bio::MAGE::AuditAndSecurity::SecurityGroup>).
...
Returns the full class name for this class, Bio::MAGE::AuditAndSecurity::SecurityGroup.

=cut

sub class_name {
  return q[Bio::MAGE::AuditAndSecurity::SecurityGroup];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::AuditAndSecurity::SecurityGroup.
...
Bio::MAGE::AuditAndSecurity::SecurityGroup: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'The members of the Security Group.',
                   '__ORDERED' => 0
...
Bio::MAGE::AuditAndSecurity::SecurityGroup: has the following association accessor methods:
...
From the MAGE-OM documentation for the C<members> association:
...
=item $array_ref = $securitygroup->setMembers($array_ref)

The restricted setter method for the members association.

Input parameters: the value to which the members association will be set : a reference to an array of objects of type C<Bio::MAGE::AuditAndSecurity::Contact>

Return value: the current value of the members association : a reference to an array of objects of type C<Bio::MAGE::AuditAndSecurity::Contact>
...
sub setMembers {
  my $self = shift;
  croak(__PACKAGE__ . "::setMembers: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setMembers: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
    croak(__PACKAGE__ . "::setMembers: expected array reference, got $self")
    unless UNIVERSAL::isa($val,'ARRAY');
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setMembers: wrong type: " . ref($val) . " expected Bio::MAGE::AuditAndSecurity::Contact")
      unless UNIVERSAL::isa($val,'Bio::MAGE::AuditAndSecurity::Contact');
  }

  return $self->{__MEMBERS} = $val;
}



=item $array_ref = $securitygroup->getMembers()

The restricted getter method for the members association.
...
sub getMembers {
  my $self = shift;
  croak(__PACKAGE__ . "::getMembers: arguments passed to getter")
    if @_;
...
=item $securitygroup->addMembers(@vals)

Because the members association has list cardinality, it may store more
than one value. This method adds the current list of objects in the members
association.

Input parameters: the list of values C<@vals> to add to the members
...
sub addMembers {
  my $self = shift;
  croak(__PACKAGE__ . "::addMembers: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
    croak(__PACKAGE__ . "::addMembers: wrong type: " . ref($val) . " expected Bio::MAGE::AuditAndSecurity::Contact")
      unless UNIVERSAL::isa($val,'Bio::MAGE::AuditAndSecurity::Contact');
  }

  push(@{$self->{__MEMBERS}},@vals);
}
