...
$VERSION = q[$Id: QuantitationType.pm,v 1.9 2002/10/13 21:56:09 jason_e_stewart Exp $];

sub initialize {
  my $self = shift;
  $self->quantitationtype_list([]);

  $self->mageml_lists([QuantitationType=>$self->quantitationtype_list]);
...
Bio::MAGE::QuantitationType - Container module for classes in the MAGE QuantitationType package 
...
=item $obj_ref = Bio::MAGE::QuantitationType->new(%parameters)
...
This method handles the list for the C<Bio::MAGE::ARRAY(0x1049b00c)::QuantitationType> class. It returns a reference to an array of the
...
=item $val = $quantitationtype->quantitationtype_list($val)

This is the unified setter/getter method for the quantitationtype_list slot. If
...
=item $val = $quantitationtype->mageml_lists()

=item $val = $quantitationtype->mageml_lists($val)
...
=item $val = $quantitationtype->tagname($val)
