...
#
##############################
package Bio::MAGE::DesignElement::FeatureReporterMap;
...
$VERSION = q[$Id: FeatureReporterMap.pm,v 1.4 2002/08/24 16:12:20 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::DesignElement::FeatureReporterMap
...
=head2 SYNOPSIS
  ...
  my $featurereportermap = Bio::MAGE::DesignElement::FeatureReporterMap->new(featureInformationSources=>$featureInformationSources_value,
			reporter=>$reporter_value);

    # setting and retrieving object associations
  my $featureInformationSources_val = $featurereportermap->featureInformationSources();
  $featurereportermap->featureInformationSources($value);

  my $reporter_val = $featurereportermap->reporter();
  $featurereportermap->reporter($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<FeatureReporterMap> class:
...
Bio::MAGE::DesignElement::FeatureReporterMap has the following super classes
...
instance of the class via the Bio::MAGE::DesignElement::FeatureReporterMap->methodname() syntax.
...
=item * featureInformationSources

Sets the value of the featureInformationSources association (from C<Bio::MAGE::DesignElement::FeatureReporterMap>).


=item * reporter

Sets the value of the reporter association (from C<Bio::MAGE::DesignElement::FeatureReporterMap>).
...
Returns the full class name for this class, Bio::MAGE::DesignElement::FeatureReporterMap.

=cut

sub class_name {
  return q[Bio::MAGE::DesignElement::FeatureReporterMap];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::DesignElement::FeatureReporterMap.
...
sub association_methods {
  my $class = shift;
  my @list = ('featureInformationSources', 'reporter');
  if ($class->superclasses()) {
    foreach ($class->superclasses()) {
...
Bio::MAGE::DesignElement::FeatureReporterMap: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   '__NAME' => 'featureInformationSources',
                   '__IS_REF' => 0,
                   '__CARDINALITY' => '1..N',
                   '__CLASS_NAME' => 'FeatureInformation',
                   '__RANK' => '2',
                   '__DESCRIPTION' => 'Typically, the features on an array that are manufactured with this reporter\'s BioSequence.',
                   '__ORDERED' => 0
                 }, 'Bio::MAGE::Association' )
        ]
...
Bio::MAGE::DesignElement::FeatureReporterMap: has the following association accessor methods:
...
From the MAGE-OM documentation for the C<featureInformationSources> association:
...
=item $array_ref = $featurereportermap->setFeatureInformationSources($array_ref)

The restricted setter method for the featureInformationSources association.

Input parameters: the value to which the featureInformationSources association will be set : a reference to an array of objects of type C<Bio::MAGE::DesignElement::FeatureInformation>

Return value: the current value of the featureInformationSources association : a reference to an array of objects of type C<Bio::MAGE::DesignElement::FeatureInformation>

Side effects: none

Exceptions: will call C<croak()> if no input parameters are specified, or
if too many input parameters are specified , or if $array_ref is not a reference to an array class C<Bio::MAGE::DesignElement::FeatureInformation> instances

=cut

sub setFeatureInformationSources {
  my $self = shift;
  croak(__PACKAGE__ . "::setFeatureInformationSources: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setFeatureInformationSources: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
    croak(__PACKAGE__ . "::setFeatureInformationSources: expected array reference, got $self")
    unless UNIVERSAL::isa($val,'ARRAY');
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setFeatureInformationSources: wrong type: " . ref($val) . " expected Bio::MAGE::DesignElement::FeatureInformation")
      unless UNIVERSAL::isa($val,'Bio::MAGE::DesignElement::FeatureInformation');
  }

  return $self->{__FEATUREINFORMATIONSOURCES} = $val;
}



=item $array_ref = $featurereportermap->getFeatureInformationSources()

The restricted getter method for the featureInformationSources association.
...
sub getFeatureInformationSources {
  my $self = shift;
  croak(__PACKAGE__ . "::getFeatureInformationSources: arguments passed to getter")
    if @_;
  return $self->{__FEATUREINFORMATIONSOURCES};
}



=item $featurereportermap->addFeatureInformationSources(@vals)

Because the featureInformationSources association has list cardinality, it may store more
than one value. This method adds the current list of objects in the featureInformationSources
association.

Input parameters: the list of values C<@vals> to add to the featureInformationSources
...
, or if any of the objects in @vals is not an instance of class C<Bio::MAGE::DesignElement::FeatureInformation>

=cut

sub addFeatureInformationSources {
  my $self = shift;
  croak(__PACKAGE__ . "::addFeatureInformationSources: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
    croak(__PACKAGE__ . "::addFeatureInformationSources: wrong type: " . ref($val) . " expected Bio::MAGE::DesignElement::FeatureInformation")
      unless UNIVERSAL::isa($val,'Bio::MAGE::DesignElement::FeatureInformation');
  }

  push(@{$self->{__FEATUREINFORMATIONSOURCES}},@vals);
}
...
=item $val = $featurereportermap->setReporter($val)
...
=item $val = $featurereportermap->getReporter()
