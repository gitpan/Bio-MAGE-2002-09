...
$VERSION = q[$Id: FeatureInformation.pm,v 1.4 2002/08/24 16:12:20 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::DesignElement::FeatureInformation
...
=head2 SYNOPSIS
  ...
  my $featureinformation = Bio::MAGE::DesignElement::FeatureInformation->new(feature=>$feature_value,
			mismatchInformation=>$mismatchInformation_value);

    # setting and retrieving object associations
  my $feature_val = $featureinformation->feature();
  $featureinformation->feature($value);

  my $mismatchInformation_val = $featureinformation->mismatchInformation();
  $featureinformation->mismatchInformation($value);
...
Sets the value of the feature association (from C<Bio::MAGE::DesignElement::FeatureInformation>).


=item * mismatchInformation

Sets the value of the mismatchInformation association (from C<Bio::MAGE::DesignElement::FeatureInformation>).
...
Returns the full class name for this class, Bio::MAGE::DesignElement::FeatureInformation.

=cut

sub class_name {
  return q[Bio::MAGE::DesignElement::FeatureInformation];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::DesignElement::FeatureInformation.
...
sub association_methods {
  my $class = shift;
  my @list = ('feature', 'mismatchInformation');
  if ($class->superclasses()) {
    foreach ($class->superclasses()) {
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'The feature the FeatureInformation is supplying information for.',
                   '__ORDERED' => 0
          ...
          bless( {
                   ...
                   '__RANK' => '2',
                   '__DESCRIPTION' => 'Differences in how the feature matches the reporter\'s sequence, typical examples is the Affymetrix probe pair where one of the features is printed with a mismatch to the other feature\'s perfect match.',
                   '__ORDERED' => 0
                 }, 'Bio::MAGE::Association' )
        ]
...
=item $featureinformation->addMismatchInformation(@vals)
