...
$VERSION = q[$Id: ReporterCompositeMap.pm,v 1.4 2002/08/24 16:12:20 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::DesignElement::ReporterCompositeMap
...
Bio::MAGE::DesignElement::ReporterCompositeMap has the following super classes
...
Returns the full class name for this class, Bio::MAGE::DesignElement::ReporterCompositeMap.

=cut

sub class_name {
  return q[Bio::MAGE::DesignElement::ReporterCompositeMap];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::DesignElement::ReporterCompositeMap.
...
Bio::MAGE::DesignElement::ReporterCompositeMap: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__RANK' => '2',
                   '__DESCRIPTION' => 'Association to the reporters that compose this CompositeSequence and where those reporters occur.',
                   '__ORDERED' => 0
                 }, 'Bio::MAGE::Association' )
        ]
...
From the MAGE-OM documentation for the C<reporterPositionSources> association:
...
The restricted setter method for the reporterPositionSources association.

Input parameters: the value to which the reporterPositionSources association will be set : a reference to an array of objects of type C<Bio::MAGE::DesignElement::ReporterPosition>

Return value: the current value of the reporterPositionSources association : a reference to an array of objects of type C<Bio::MAGE::DesignElement::ReporterPosition>

Side effects: none

Exceptions: will call C<croak()> if no input parameters are specified, or
if too many input parameters are specified , or if $array_ref is not a reference to an array class C<Bio::MAGE::DesignElement::ReporterPosition> instances
...
sub setReporterPositionSources {
  ...
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setReporterPositionSources: wrong type: " . ref($val) . " expected Bio::MAGE::DesignElement::ReporterPosition")
...
The restricted getter method for the reporterPositionSources association.
...
=item $reportercompositemap->addReporterPositionSources(@vals)

Because the reporterPositionSources association has list cardinality, it may store more
than one value. This method adds the current list of objects in the reporterPositionSources
association.

Input parameters: the list of values C<@vals> to add to the reporterPositionSources
...
, or if any of the objects in @vals is not an instance of class C<Bio::MAGE::DesignElement::ReporterPosition>

=cut

sub addReporterPositionSources {
  my $self = shift;
  croak(__PACKAGE__ . "::addReporterPositionSources: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
