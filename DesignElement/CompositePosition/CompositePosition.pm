...
#
##############################
package Bio::MAGE::DesignElement::CompositePosition;
...
$VERSION = q[$Id: CompositePosition.pm,v 1.4 2002/08/24 16:12:19 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::DesignElement::CompositePosition
...
=head2 SYNOPSIS
  ...
  my $compositeposition = Bio::MAGE::DesignElement::CompositePosition->new(composite=>$composite_value,
			mismatchInformation=>$mismatchInformation_value);

    # setting and retrieving object associations
  my $composite_val = $compositeposition->composite();
  $compositeposition->composite($value);

  my $mismatchInformation_val = $compositeposition->mismatchInformation();
  $compositeposition->mismatchInformation($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<CompositePosition> class:
...
Bio::MAGE::DesignElement::CompositePosition has the following super classes
...
instance of the class via the Bio::MAGE::DesignElement::CompositePosition->methodname() syntax.
...
Sets the value of the composite association (from C<Bio::MAGE::DesignElement::CompositePosition>).


=item * mismatchInformation

Sets the value of the mismatchInformation association (from C<Bio::MAGE::DesignElement::CompositePosition>).
...
Returns the full class name for this class, Bio::MAGE::DesignElement::CompositePosition.

=cut

sub class_name {
  return q[Bio::MAGE::DesignElement::CompositePosition];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::DesignElement::CompositePosition.
...
sub association_methods {
  my $class = shift;
  my @list = ('composite', 'mismatchInformation');
  if ($class->superclasses()) {
    foreach ($class->superclasses()) {
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'A source CompositeSequence that is part of a target CompositeSequence',
                   '__ORDERED' => 0
          ...
          bless( {
                   ...
                   '__RANK' => '2',
                   '__DESCRIPTION' => 'Differences in how the contained compositeSequence matches its target compositeSequence\'s sequence.',
                   '__ORDERED' => 0
                 }, 'Bio::MAGE::Association' )
        ]
...
From the MAGE-OM documentation for the C<composite> association:
...
=item $val = $compositeposition->setComposite($val)

The restricted setter method for the composite association.

Input parameters: the value to which the composite association will be set : an instance of type C<Bio::MAGE::DesignElement::CompositeSequence>.

Return value: the current value of the composite association : an instance of type C<Bio::MAGE::DesignElement::CompositeSequence>.
...
sub setComposite {
  my $self = shift;
  croak(__PACKAGE__ . "::setComposite: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setComposite: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
  croak(__PACKAGE__ . "::setComposite: wrong type: " . ref($val) . " expected Bio::MAGE::DesignElement::CompositeSequence") unless UNIVERSAL::isa($val,'Bio::MAGE::DesignElement::CompositeSequence');
  return $self->{__COMPOSITE} = $val;
}



=item $val = $compositeposition->getComposite()
...
sub getComposite {
  my $self = shift;
  croak(__PACKAGE__ . "::getComposite: arguments passed to getter")
    if @_;
...
=item $array_ref = $compositeposition->setMismatchInformation($array_ref)
...
=item $array_ref = $compositeposition->getMismatchInformation()
...
=item $compositeposition->addMismatchInformation(@vals)
