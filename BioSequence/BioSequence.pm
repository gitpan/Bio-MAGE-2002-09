...
$VERSION = q[$Id: BioSequence.pm,v 1.8 2002/10/13 21:56:04 jason_e_stewart Exp $];

sub initialize {
  my $self = shift;
  $self->biosequence_list([]);

  $self->mageml_lists([BioSequence=>$self->biosequence_list]);
  $self->tagname(q[BioSequence_package]);
  return 1;
}
...
Bio::MAGE::BioSequence - Container module for classes in the MAGE BioSequence package 
...
BioSequence package of the MAGE-OM object model.

=head1 CLASSES

The Bio::MAGE::BioSequence module contains the following
Bio::MAGE classes:
...
=item @class_list = Bio::MAGE::BioSequence::classes();
...
sub classes {
  return ('SeqFeature', 'SeqFeatureLocation', 'BioSequence', 'SequencePosition');
}

=item $obj_ref = Bio::MAGE::BioSequence->new(%parameters)
...
=item $array_ref = $biosequence->getBioSequence_list()

This method handles the list for the C<Bio::MAGE::ARRAY(0x1048d428)::BioSequence> class. It returns a reference to an array of the
...
sub getBioSequence_list {
  my $self = shift;
  return $self->biosequence_list();
}
...
=item $val = $biosequence->biosequence_list()

=item $val = $biosequence->biosequence_list($val)

This is the unified setter/getter method for the biosequence_list slot. If
...
Return value: for both setter and getter the current value of the biosequence_list
slot
...
sub biosequence_list {
  my $self = shift;
  if (@_) {
    $self->{__BIOSEQUENCE_LIST} = shift;
  }
  return $self->{__BIOSEQUENCE_LIST};
}

=item $val = $biosequence->mageml_lists()

=item $val = $biosequence->mageml_lists($val)
...
=item $val = $biosequence->tagname()

=item $val = $biosequence->tagname($val)
...
sub register {
  ...
  foreach my $class (keys %mageml_lists) {
    if ($obj->isa("Bio::MAGE::BioSequence::$class")) {
