...
$VERSION = q[$Id: SeqFeature.pm,v 1.4 2002/08/24 16:12:18 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::BioSequence::SeqFeature
...
=head2 SYNOPSIS
  ...
  my $seqfeature = Bio::MAGE::BioSequence::SeqFeature->new(basis=>$basis_value,
			regions=>$regions_value);

    # setting and retrieving object attributes
  my $basis_val = $seqfeature->basis();
  $seqfeature->basis($value);

    # setting and retrieving object associations
  my $regions_val = $seqfeature->regions();
  $seqfeature->regions($value);
...
Bio::MAGE::BioSequence::SeqFeature has the following super classes
...
instance of the class via the Bio::MAGE::BioSequence::SeqFeature->methodname() syntax.
...
Sets the value of the basis attribute (from C<Bio::MAGE::BioSequence::SeqFeature>).


=item * regions

Sets the value of the regions association (from C<Bio::MAGE::BioSequence::SeqFeature>).
...
Returns the full class name for this class, Bio::MAGE::BioSequence::SeqFeature.

=cut

sub class_name {
  return q[Bio::MAGE::BioSequence::SeqFeature];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::BioSequence::SeqFeature.
...
Bio::MAGE::BioSequence::SeqFeature: has the following attribute accessor methods:
...
From the MAGE-OM documentation for the C<basis> attribute:
...
=item $val = $seqfeature->setBasis($val)

The restricted setter method for the basis attribute.

Input parameters: the value to which the basis attribute will be set 

Return value: the current value of the basis attribute 
...
sub setBasis {
  my $self = shift;
  croak(__PACKAGE__ . "::setBasis: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setBasis: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
  
  return $self->{__BASIS} = $val;
}



=item $val = $seqfeature->getBasis()

The restricted getter method for the basis attribute.
...
sub getBasis {
  my $self = shift;
  croak(__PACKAGE__ . "::getBasis: arguments passed to getter")
    if @_;
  return $self->{__BASIS};
}
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'Association to classes that describe the location with the sequence of the SeqFeature.',
                   '__ORDERED' => 0
...
Bio::MAGE::BioSequence::SeqFeature: has the following association accessor methods:
...
From the MAGE-OM documentation for the C<regions> association:
...
=item $array_ref = $seqfeature->setRegions($array_ref)

The restricted setter method for the regions association.

Input parameters: the value to which the regions association will be set : a reference to an array of objects of type C<Bio::MAGE::BioSequence::SeqFeatureLocation>

Return value: the current value of the regions association : a reference to an array of objects of type C<Bio::MAGE::BioSequence::SeqFeatureLocation>
...
sub setRegions {
  my $self = shift;
  croak(__PACKAGE__ . "::setRegions: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setRegions: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
    croak(__PACKAGE__ . "::setRegions: expected array reference, got $self")
    unless UNIVERSAL::isa($val,'ARRAY');
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setRegions: wrong type: " . ref($val) . " expected Bio::MAGE::BioSequence::SeqFeatureLocation")
      unless UNIVERSAL::isa($val,'Bio::MAGE::BioSequence::SeqFeatureLocation');
  }

  return $self->{__REGIONS} = $val;
}



=item $array_ref = $seqfeature->getRegions()

The restricted getter method for the regions association.
...
sub getRegions {
  my $self = shift;
  croak(__PACKAGE__ . "::getRegions: arguments passed to getter")
    if @_;
...
=item $seqfeature->addRegions(@vals)

Because the regions association has list cardinality, it may store more
than one value. This method adds the current list of objects in the regions
association.

Input parameters: the list of values C<@vals> to add to the regions
...
sub addRegions {
  my $self = shift;
  croak(__PACKAGE__ . "::addRegions: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
    croak(__PACKAGE__ . "::addRegions: wrong type: " . ref($val) . " expected Bio::MAGE::BioSequence::SeqFeatureLocation")
      unless UNIVERSAL::isa($val,'Bio::MAGE::BioSequence::SeqFeatureLocation');
  }

  push(@{$self->{__REGIONS}},@vals);
}
