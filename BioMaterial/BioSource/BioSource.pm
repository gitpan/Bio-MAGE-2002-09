...
#
##############################
package Bio::MAGE::BioMaterial::BioSource;
...
$VERSION = q[$Id: BioSource.pm,v 1.4 2002/08/24 16:12:17 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::BioMaterial::BioSource
...
=head2 SYNOPSIS
  ...
  my $biosource = Bio::MAGE::BioMaterial::BioSource->new(sourceContact=>$sourceContact_value);

    # setting and retrieving object associations
  my $sourceContact_val = $biosource->sourceContact();
  $biosource->sourceContact($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<BioSource> class:
...
Bio::MAGE::BioMaterial::BioSource has the following super classes
...
instance of the class via the Bio::MAGE::BioMaterial::BioSource->methodname() syntax.
...
Sets the value of the sourceContact association (from C<Bio::MAGE::BioMaterial::BioSource>).
...
Returns the full class name for this class, Bio::MAGE::BioMaterial::BioSource.

=cut

sub class_name {
  return q[Bio::MAGE::BioMaterial::BioSource];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::BioMaterial::BioSource.
...
Bio::MAGE::BioMaterial::BioSource: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          'sourceContact',
          bless( {
                   '__NAME' => 'sourceContact',
                   '__IS_REF' => 1,
                   '__CARDINALITY' => '0..N',
                   '__CLASS_NAME' => 'Contact',
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'The BioSource\'s source is the provider of the biological material (a cell line, strain, etc...).  This could be the ATTC (American Tissue Type Collection).',
                   '__ORDERED' => 0
...
Bio::MAGE::BioMaterial::BioSource: has the following association accessor methods:
...
From the MAGE-OM documentation for the C<sourceContact> association:
...
=item $array_ref = $biosource->setSourceContact($array_ref)

The restricted setter method for the sourceContact association.

Input parameters: the value to which the sourceContact association will be set : a reference to an array of objects of type C<Bio::MAGE::AuditAndSecurity::Contact>

Return value: the current value of the sourceContact association : a reference to an array of objects of type C<Bio::MAGE::AuditAndSecurity::Contact>
...
sub setSourceContact {
  my $self = shift;
  croak(__PACKAGE__ . "::setSourceContact: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setSourceContact: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
    croak(__PACKAGE__ . "::setSourceContact: expected array reference, got $self")
    unless UNIVERSAL::isa($val,'ARRAY');
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setSourceContact: wrong type: " . ref($val) . " expected Bio::MAGE::AuditAndSecurity::Contact")
      unless UNIVERSAL::isa($val,'Bio::MAGE::AuditAndSecurity::Contact');
  }

  return $self->{__SOURCECONTACT} = $val;
}



=item $array_ref = $biosource->getSourceContact()

The restricted getter method for the sourceContact association.
...
sub getSourceContact {
  my $self = shift;
  croak(__PACKAGE__ . "::getSourceContact: arguments passed to getter")
    if @_;
  return $self->{__SOURCECONTACT};
}



=item $biosource->addSourceContact(@vals)

Because the sourceContact association has list cardinality, it may store more
than one value. This method adds the current list of objects in the sourceContact
association.

Input parameters: the list of values C<@vals> to add to the sourceContact
...
sub addSourceContact {
  my $self = shift;
  croak(__PACKAGE__ . "::addSourceContact: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
    croak(__PACKAGE__ . "::addSourceContact: wrong type: " . ref($val) . " expected Bio::MAGE::AuditAndSecurity::Contact")
      unless UNIVERSAL::isa($val,'Bio::MAGE::AuditAndSecurity::Contact');
  }

  push(@{$self->{__SOURCECONTACT}},@vals);
}
