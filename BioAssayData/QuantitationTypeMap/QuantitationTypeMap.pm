...
#
##############################
package Bio::MAGE::BioAssayData::QuantitationTypeMap;
...
$VERSION = q[$Id: QuantitationTypeMap.pm,v 1.4 2002/08/24 16:12:16 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::BioAssayData::QuantitationTypeMap
...
=head2 SYNOPSIS
  ...
  my $quantitationtypemap = Bio::MAGE::BioAssayData::QuantitationTypeMap->new(sourcesQuantitationType=>$sourcesQuantitationType_value,
			targetQuantitationType=>$targetQuantitationType_value);

    # setting and retrieving object associations
  my $sourcesQuantitationType_val = $quantitationtypemap->sourcesQuantitationType();
  $quantitationtypemap->sourcesQuantitationType($value);

  my $targetQuantitationType_val = $quantitationtypemap->targetQuantitationType();
  $quantitationtypemap->targetQuantitationType($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<QuantitationTypeMap> class:
...
Bio::MAGE::BioAssayData::QuantitationTypeMap has the following super classes
...
instance of the class via the Bio::MAGE::BioAssayData::QuantitationTypeMap->methodname() syntax.
...
=item * sourcesQuantitationType

Sets the value of the sourcesQuantitationType association (from C<Bio::MAGE::BioAssayData::QuantitationTypeMap>).


=item * targetQuantitationType

Sets the value of the targetQuantitationType association (from C<Bio::MAGE::BioAssayData::QuantitationTypeMap>).
...
Returns the full class name for this class, Bio::MAGE::BioAssayData::QuantitationTypeMap.

=cut

sub class_name {
  return q[Bio::MAGE::BioAssayData::QuantitationTypeMap];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::BioAssayData::QuantitationTypeMap.
...
sub association_methods {
  my $class = shift;
  my @list = ('sourcesQuantitationType', 'targetQuantitationType');
  if ($class->superclasses()) {
    foreach ($class->superclasses()) {
...
Bio::MAGE::BioAssayData::QuantitationTypeMap: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   '__NAME' => 'sourcesQuantitationType',
                   '__IS_REF' => 1,
                   '__CARDINALITY' => '0..N',
                   '__CLASS_NAME' => 'QuantitationType',
                   '__RANK' => '2',
                   '__DESCRIPTION' => 'The QuantitationType sources for values for the transformation.',
                   '__ORDERED' => 0
                 }, 'Bio::MAGE::Association' )
        ]
...
Bio::MAGE::BioAssayData::QuantitationTypeMap: has the following association accessor methods:
...
From the MAGE-OM documentation for the C<sourcesQuantitationType> association:
...
=item $array_ref = $quantitationtypemap->setSourcesQuantitationType($array_ref)

The restricted setter method for the sourcesQuantitationType association.

Input parameters: the value to which the sourcesQuantitationType association will be set : a reference to an array of objects of type C<Bio::MAGE::QuantitationType::QuantitationType>

Return value: the current value of the sourcesQuantitationType association : a reference to an array of objects of type C<Bio::MAGE::QuantitationType::QuantitationType>

Side effects: none

Exceptions: will call C<croak()> if no input parameters are specified, or
if too many input parameters are specified , or if $array_ref is not a reference to an array class C<Bio::MAGE::QuantitationType::QuantitationType> instances

=cut

sub setSourcesQuantitationType {
  my $self = shift;
  croak(__PACKAGE__ . "::setSourcesQuantitationType: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setSourcesQuantitationType: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
    croak(__PACKAGE__ . "::setSourcesQuantitationType: expected array reference, got $self")
    unless UNIVERSAL::isa($val,'ARRAY');
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setSourcesQuantitationType: wrong type: " . ref($val) . " expected Bio::MAGE::QuantitationType::QuantitationType")
      unless UNIVERSAL::isa($val,'Bio::MAGE::QuantitationType::QuantitationType');
  }

  return $self->{__SOURCESQUANTITATIONTYPE} = $val;
}



=item $array_ref = $quantitationtypemap->getSourcesQuantitationType()

The restricted getter method for the sourcesQuantitationType association.
...
sub getSourcesQuantitationType {
  my $self = shift;
  croak(__PACKAGE__ . "::getSourcesQuantitationType: arguments passed to getter")
    if @_;
  return $self->{__SOURCESQUANTITATIONTYPE};
}



=item $quantitationtypemap->addSourcesQuantitationType(@vals)

Because the sourcesQuantitationType association has list cardinality, it may store more
than one value. This method adds the current list of objects in the sourcesQuantitationType
association.

Input parameters: the list of values C<@vals> to add to the sourcesQuantitationType
...
, or if any of the objects in @vals is not an instance of class C<Bio::MAGE::QuantitationType::QuantitationType>

=cut

sub addSourcesQuantitationType {
  my $self = shift;
  croak(__PACKAGE__ . "::addSourcesQuantitationType: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
    croak(__PACKAGE__ . "::addSourcesQuantitationType: wrong type: " . ref($val) . " expected Bio::MAGE::QuantitationType::QuantitationType")
      unless UNIVERSAL::isa($val,'Bio::MAGE::QuantitationType::QuantitationType');
  }

  push(@{$self->{__SOURCESQUANTITATIONTYPE}},@vals);
}
...
=item $val = $quantitationtypemap->setTargetQuantitationType($val)
...
=item $val = $quantitationtypemap->getTargetQuantitationType()
