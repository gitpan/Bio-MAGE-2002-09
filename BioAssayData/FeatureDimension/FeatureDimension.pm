...
#
##############################
package Bio::MAGE::BioAssayData::FeatureDimension;
...
$VERSION = q[$Id: FeatureDimension.pm,v 1.4 2002/08/24 16:12:16 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::BioAssayData::FeatureDimension
...
=head2 SYNOPSIS
  ...
  my $featuredimension = Bio::MAGE::BioAssayData::FeatureDimension->new(containedFeatures=>$containedFeatures_value);

    # setting and retrieving object associations
  my $containedFeatures_val = $featuredimension->containedFeatures();
  $featuredimension->containedFeatures($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<FeatureDimension> class:
...
Bio::MAGE::BioAssayData::FeatureDimension has the following super classes
...
instance of the class via the Bio::MAGE::BioAssayData::FeatureDimension->methodname() syntax.
...
=item * containedFeatures

Sets the value of the containedFeatures association (from C<Bio::MAGE::BioAssayData::FeatureDimension>).
...
Returns the full class name for this class, Bio::MAGE::BioAssayData::FeatureDimension.

=cut

sub class_name {
  return q[Bio::MAGE::BioAssayData::FeatureDimension];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::BioAssayData::FeatureDimension.
...
Bio::MAGE::BioAssayData::FeatureDimension: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          'containedFeatures',
          bless( {
                   '__NAME' => 'containedFeatures',
                   '__IS_REF' => 1,
                   '__CARDINALITY' => '0..N',
                   '__CLASS_NAME' => 'Feature',
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'The features for this dimension.',
                   '__ORDERED' => 1
...
Bio::MAGE::BioAssayData::FeatureDimension: has the following association accessor methods:
...
From the MAGE-OM documentation for the C<containedFeatures> association:
...
=item $array_ref = $featuredimension->setContainedFeatures($array_ref)

The restricted setter method for the containedFeatures association.

Input parameters: the value to which the containedFeatures association will be set : a reference to an array of objects of type C<Bio::MAGE::DesignElement::Feature>

Return value: the current value of the containedFeatures association : a reference to an array of objects of type C<Bio::MAGE::DesignElement::Feature>
...
sub setContainedFeatures {
  my $self = shift;
  croak(__PACKAGE__ . "::setContainedFeatures: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setContainedFeatures: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
    croak(__PACKAGE__ . "::setContainedFeatures: expected array reference, got $self")
    unless UNIVERSAL::isa($val,'ARRAY');
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setContainedFeatures: wrong type: " . ref($val) . " expected Bio::MAGE::DesignElement::Feature")
      unless UNIVERSAL::isa($val,'Bio::MAGE::DesignElement::Feature');
  }

  return $self->{__CONTAINEDFEATURES} = $val;
}



=item $array_ref = $featuredimension->getContainedFeatures()

The restricted getter method for the containedFeatures association.
...
sub getContainedFeatures {
  my $self = shift;
  croak(__PACKAGE__ . "::getContainedFeatures: arguments passed to getter")
    if @_;
  return $self->{__CONTAINEDFEATURES};
}



=item $featuredimension->addContainedFeatures(@vals)

Because the containedFeatures association has list cardinality, it may store more
than one value. This method adds the current list of objects in the containedFeatures
association.

Input parameters: the list of values C<@vals> to add to the containedFeatures
...
sub addContainedFeatures {
  my $self = shift;
  croak(__PACKAGE__ . "::addContainedFeatures: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
    croak(__PACKAGE__ . "::addContainedFeatures: wrong type: " . ref($val) . " expected Bio::MAGE::DesignElement::Feature")
      unless UNIVERSAL::isa($val,'Bio::MAGE::DesignElement::Feature');
  }

  push(@{$self->{__CONTAINEDFEATURES}},@vals);
}
