...
$VERSION = q[$Id: BioEvent.pm,v 1.7 2002/10/13 21:56:04 jason_e_stewart Exp $];

sub initialize {
  my $self = shift;

  $self->mageml_lists([]);
  $self->tagname(q[BioEvent_package]);
  return 1;
}
...
Bio::MAGE::BioEvent - Container module for classes in the MAGE BioEvent package 
...
BioEvent package of the MAGE-OM object model.

=head1 CLASSES

The Bio::MAGE::BioEvent module contains the following
Bio::MAGE classes:
...
=item @class_list = Bio::MAGE::BioEvent::classes();
...
=item $obj_ref = Bio::MAGE::BioEvent->new(%parameters)
...
=item $val = $bioevent->mageml_lists()

=item $val = $bioevent->mageml_lists($val)
...
=item $val = $bioevent->tagname()

=item $val = $bioevent->tagname($val)
...
sub register {
  ...
  foreach my $class (keys %mageml_lists) {
    if ($obj->isa("Bio::MAGE::BioEvent::$class")) {
