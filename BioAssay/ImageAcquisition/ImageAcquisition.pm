...
#
##############################
package Bio::MAGE::BioAssay::ImageAcquisition;
...
@ISA = qw(Bio::MAGE::Base Bio::MAGE::BioAssay::BioAssayTreatment Exporter);
$VERSION = q[$Id: ImageAcquisition.pm,v 1.4 2002/08/24 16:12:15 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::BioAssay::ImageAcquisition
...
=head2 SYNOPSIS
  ...
  my $imageacquisition = Bio::MAGE::BioAssay::ImageAcquisition->new(images=>$images_value);

    # setting and retrieving object associations
  my $images_val = $imageacquisition->images();
  $imageacquisition->images($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<ImageAcquisition> class:
...
Bio::MAGE::BioAssay::ImageAcquisition has the following super classes

=over 


=item * Bio::MAGE::BioAssay::BioAssayTreatment
...
instance of the class via the Bio::MAGE::BioAssay::ImageAcquisition->methodname() syntax.
...
Sets the value of the images association (from C<Bio::MAGE::BioAssay::ImageAcquisition>).
...
Returns the full class name for this class, Bio::MAGE::BioAssay::ImageAcquisition.

=cut

sub class_name {
  return q[Bio::MAGE::BioAssay::ImageAcquisition];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::BioAssay::ImageAcquisition.
...
sub superclasses {
  my @list = ('Bio::MAGE::BioAssay::BioAssayTreatment');
  return @list;
}
...
Bio::MAGE::BioAssay::ImageAcquisition: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__CLASS_NAME' => 'Image',
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'The images produced by the ImageAcquisition event.',
                   '__ORDERED' => 0
...
Bio::MAGE::BioAssay::ImageAcquisition: has the following association accessor methods:
...
From the MAGE-OM documentation for the C<images> association:
...
=item $array_ref = $imageacquisition->setImages($array_ref)

The restricted setter method for the images association.

Input parameters: the value to which the images association will be set : a reference to an array of objects of type C<Bio::MAGE::BioAssay::Image>

Return value: the current value of the images association : a reference to an array of objects of type C<Bio::MAGE::BioAssay::Image>
...
sub setImages {
  my $self = shift;
  croak(__PACKAGE__ . "::setImages: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setImages: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
    croak(__PACKAGE__ . "::setImages: expected array reference, got $self")
    unless UNIVERSAL::isa($val,'ARRAY');
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setImages: wrong type: " . ref($val) . " expected Bio::MAGE::BioAssay::Image")
      unless UNIVERSAL::isa($val,'Bio::MAGE::BioAssay::Image');
  }

  return $self->{__IMAGES} = $val;
}



=item $array_ref = $imageacquisition->getImages()

The restricted getter method for the images association.
...
sub getImages {
  my $self = shift;
  croak(__PACKAGE__ . "::getImages: arguments passed to getter")
    if @_;
...
=item $imageacquisition->addImages(@vals)

Because the images association has list cardinality, it may store more
than one value. This method adds the current list of objects in the images
association.

Input parameters: the list of values C<@vals> to add to the images
...
sub addImages {
  my $self = shift;
  croak(__PACKAGE__ . "::addImages: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
    croak(__PACKAGE__ . "::addImages: wrong type: " . ref($val) . " expected Bio::MAGE::BioAssay::Image")
      unless UNIVERSAL::isa($val,'Bio::MAGE::BioAssay::Image');
  }

  push(@{$self->{__IMAGES}},@vals);
}
...
Associations Inherited from Bio::MAGE::BioAssay::BioAssayTreatment
