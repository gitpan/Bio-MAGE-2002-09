...
#
##############################
package Bio::MAGE::BioAssay::BioAssay;
...
$VERSION = q[$Id: BioAssay.pm,v 1.7 2002/10/13 21:56:04 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::BioAssay::BioAssay
...
=head2 SYNOPSIS
  ...
  my $bioassay = Bio::MAGE::BioAssay::BioAssay->new(bioAssayFactorValues=>$bioAssayFactorValues_value,
			channels=>$channels_value);

    # setting and retrieving object associations
  my $bioAssayFactorValues_val = $bioassay->bioAssayFactorValues();
  $bioassay->bioAssayFactorValues($value);

  my $channels_val = $bioassay->channels();
  $bioassay->channels($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<BioAssay> class:
...
Bio::MAGE::BioAssay::BioAssay has the following super classes
...
Bio::MAGE::BioAssay::BioAssay has the following subclasses

=over 


=item * Bio::MAGE::BioAssay::MeasuredBioAssay


=item * Bio::MAGE::BioAssay::DerivedBioAssay


=item * Bio::MAGE::BioAssay::PhysicalBioAssay
...
instance of the class via the Bio::MAGE::BioAssay::BioAssay->methodname() syntax.
...
Returns the full class name for this class, Bio::MAGE::BioAssay::BioAssay.

=cut

sub class_name {
  return q[Bio::MAGE::BioAssay::BioAssay];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::BioAssay::BioAssay.
...
sub subclasses {
  my @list = ('Bio::MAGE::BioAssay::MeasuredBioAssay', 'Bio::MAGE::BioAssay::DerivedBioAssay', 'Bio::MAGE::BioAssay::PhysicalBioAssay');
  return @list;
}
...
sub association_methods {
  my $class = shift;
  my @list = ('bioAssayFactorValues', 'channels');
  if ($class->superclasses()) {
    foreach ($class->superclasses()) {
...
Bio::MAGE::BioAssay::BioAssay: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'Channels can be non-null for all subclasses.  For instance, collapsing across replicate features will create a DerivedBioAssay that will potentially reference channels.',
                   '__ORDERED' => 0
                 }, 'Bio::MAGE::Association' ),
          'bioAssayFactorValues',
          bless( {
                   '__NAME' => 'bioAssayFactorValues',
                   '__IS_REF' => 1,
                   '__CARDINALITY' => '0..N',
                   '__CLASS_NAME' => 'FactorValue',
                   '__RANK' => '2',
                   '__DESCRIPTION' => 'The values that this BioAssay is associated with for the experiment.',
                   '__ORDERED' => 0
                 }, 'Bio::MAGE::Association' )
        ]
...
Bio::MAGE::BioAssay::BioAssay: has the following association accessor methods:
...
sub setBioAssayFactorValues {
  ...
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setBioAssayFactorValues: wrong type: " . ref($val) . " expected Bio::MAGE::Experiment::FactorValue")
...
sub addBioAssayFactorValues {
  my $self = shift;
  croak(__PACKAGE__ . "::addBioAssayFactorValues: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
  ...
  push(@{$self->{__BIOASSAYFACTORVALUES}},@vals);
}
...
sub initialize {
  carp "Bio::MAGE::BioAssay::BioAssay::initialize: abstract base classes should not be instantiated";
}
