...
#
##############################
package Bio::MAGE::Protocol::HardwareApplication;
...
$VERSION = q[$Id: HardwareApplication.pm,v 1.4 2002/08/24 16:12:22 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::Protocol::HardwareApplication
...
=head2 SYNOPSIS
  ...
  my $hardwareapplication = Bio::MAGE::Protocol::HardwareApplication->new(serialNumber=>$serialNumber_value,
			hardware=>$hardware_value);

    # setting and retrieving object attributes
  my $serialNumber_val = $hardwareapplication->serialNumber();
  $hardwareapplication->serialNumber($value);

    # setting and retrieving object associations
  my $hardware_val = $hardwareapplication->hardware();
  $hardwareapplication->hardware($value);


=head2 DESCRIPTION

From the MAGE-OM documentation for the C<HardwareApplication> class:
...
Bio::MAGE::Protocol::HardwareApplication has the following super classes
...
instance of the class via the Bio::MAGE::Protocol::HardwareApplication->methodname() syntax.
...
Sets the value of the serialNumber attribute (from C<Bio::MAGE::Protocol::HardwareApplication>).


=item * hardware

Sets the value of the hardware association (from C<Bio::MAGE::Protocol::HardwareApplication>).
...
Returns the full class name for this class, Bio::MAGE::Protocol::HardwareApplication.

=cut

sub class_name {
  return q[Bio::MAGE::Protocol::HardwareApplication];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::Protocol::HardwareApplication.
...
Bio::MAGE::Protocol::HardwareApplication: has the following attribute accessor methods:
...
From the MAGE-OM documentation for the C<serialNumber> attribute:
...
=item $val = $hardwareapplication->setSerialNumber($val)

The restricted setter method for the serialNumber attribute.

Input parameters: the value to which the serialNumber attribute will be set 

Return value: the current value of the serialNumber attribute 
...
sub setSerialNumber {
  my $self = shift;
  croak(__PACKAGE__ . "::setSerialNumber: no arguments passed to setter")
    unless @_;
  croak(__PACKAGE__ . "::setSerialNumber: too many arguments passed to setter")
    if @_ > 1;
  my $val = shift;
  
  return $self->{__SERIALNUMBER} = $val;
}



=item $val = $hardwareapplication->getSerialNumber()

The restricted getter method for the serialNumber attribute.
...
sub getSerialNumber {
  my $self = shift;
  croak(__PACKAGE__ . "::getSerialNumber: arguments passed to getter")
    if @_;
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'The underlying hardware.',
                   '__ORDERED' => 0
...
Bio::MAGE::Protocol::HardwareApplication: has the following association accessor methods:
...
=item $val = $hardwareapplication->setHardware($val)
...
=item $val = $hardwareapplication->getHardware()
