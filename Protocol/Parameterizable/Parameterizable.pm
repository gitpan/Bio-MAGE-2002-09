...
$VERSION = q[$Id: Parameterizable.pm,v 1.4 2002/08/24 16:12:22 jason_e_stewart Exp $];
...
=head1 Bio::MAGE::Protocol::Parameterizable
...
=head2 SYNOPSIS
  ...
  my $parameterizable = Bio::MAGE::Protocol::Parameterizable->new(URI=>$URI_value,
			parameterTypes=>$parameterTypes_value);

    # setting and retrieving object attributes
  my $URI_val = $parameterizable->URI();
  $parameterizable->URI($value);

    # setting and retrieving object associations
  my $parameterTypes_val = $parameterizable->parameterTypes();
  $parameterizable->parameterTypes($value);
...
Bio::MAGE::Protocol::Parameterizable has the following super classes
...
Returns the full class name for this class, Bio::MAGE::Protocol::Parameterizable.

=cut

sub class_name {
  return q[Bio::MAGE::Protocol::Parameterizable];
}

=item $package_name = package()

Returns the unresolved package name (i.e. no 'Bio::MAGE::') of the
package that contains class, Bio::MAGE::Protocol::Parameterizable.
...
sub subclasses {
  my @list = ('Bio::MAGE::Protocol::Protocol', 'Bio::MAGE::Protocol::Hardware', 'Bio::MAGE::Protocol::Software');
  return @list;
}
...
Bio::MAGE::Protocol::Parameterizable: has the following attribute accessor methods:
...
BEGIN {
  $__ASSOCIATIONS = [
          ...
          bless( {
                   ...
                   '__RANK' => '1',
                   '__DESCRIPTION' => 'The description of the parameters for the Parameterizable class instance.',
                   '__ORDERED' => 0
...
sub setParameterTypes {
  ...
  foreach my $val (@{$val}) {
    croak(__PACKAGE__ . "::setParameterTypes: wrong type: " . ref($val) . " expected Bio::MAGE::Protocol::Parameter")
...
sub addParameterTypes {
  my $self = shift;
  croak(__PACKAGE__ . "::addParameterTypes: no arguments passed to setter")
    unless @_;
  my @vals = @_;
    foreach my $val (@vals) {
  ...
  push(@{$self->{__PARAMETERTYPES}},@vals);
}
...
sub initialize {
  carp "Bio::MAGE::Protocol::Parameterizable::initialize: abstract base classes should not be instantiated";
}
